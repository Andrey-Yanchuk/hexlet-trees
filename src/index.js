...
/*-----------------------------------------------------*/
const getFileSize = (tree) => {
  const meta = getMeta(tree);
  if (isFile(tree)) return meta.size;
  const children = getChildren(tree);
  return children.reduce((acc, child) => acc + getFileSize(child), 0);
};
export const du = (tree) => {
  const children = getChildren(tree);
  const result = children.map((child) => [getName(child), getFileSize(child)]);
  return result.sort((a, b) => b[1] - a[1]);
};
/*-----------------------------------------------------*/
export const findFilesByName = (tree, substr = "") => {
  const iter = (node, ancestry) => {
    const name = getName(node);
    const newPath = path.join(ancestry, name);
    if (isFile(node) && name.includes(substr)) return [newPath];
